module ApplicationHelper
  ...
  def heatmap(histogram = {})
    ...
    histogram.keys.sort{|a,b| histogram[a] <=> histogram[b]}.reverse.each do |k|
      ...
      _size = (((histogram[k] / histogram.map{|key,val| histogram[key]}.sum.to_f) * 100) ).to_i
      _heat = sprintf("%02x" % (255 - (_size * 25)))
      html << %{
        <div class="heatmap_element" style="color: ##{_heat}#{_heat}#{_heat}; font-size: #{_size}px; height: #{_max+_size}px;"><a href=\"/tag/#{k}\">#{k}</a></div>
      }
    end
